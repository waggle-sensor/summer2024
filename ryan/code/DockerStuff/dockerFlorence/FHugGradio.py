...
models = {
    ...
    'microsoft/Florence-2-base': AutoModelForCausalLM.from_pretrained('microsoft/Florence-2-base', trust_remote_code=True).to(device).eval(),
}
...
def group_components(components_from_description, boxes_from_description, identified_components, identified_component_boxes):
    # Initialize grouped_components dictionary
    grouped_components = {
        'labels': [],
        'bboxes': []
    }

    # Group components from description into 'labels' and 'bboxes'
    for component, box in zip(components_from_description, boxes_from_description):
        grouped_components['labels'].append(component)
        grouped_components['bboxes'].append(box)

    # Group identified components into 'labels' and 'bboxes'
...
def process_image(image, task_prompt, text_input=None, model_id='microsoft/Florence-2-base'):
    ...
    if task_prompt == 'Caption':
        ...
        return results, fig_to_pil(fig)
    #new version
    elif task_prompt == 'More Detailed Caption + Grounding + Dense Region Caption':
         task_prompt = '<MORE_DETAILED_CAPTION>'
         results = run_example(task_prompt, image, model_id=model_id)
         text_input = results[task_prompt]
         task_prompt = '<CAPTION_TO_PHRASE_GROUNDING>'
         boxed_descriptions = run_example(task_prompt, image, text_input, model_id)
         components_from_description = boxed_descriptions[task_prompt]['labels']
         boxes_from_description = boxed_descriptions[task_prompt]['bboxes']


         labels = run_example('<DENSE_REGION_CAPTION>', image)
         identified_components = labels['<DENSE_REGION_CAPTION>']['labels']
         ...
         fig = plot_bbox(image, grouped_components)


         #results = grouped_components.update(results)
         results.update(grouped_components)
...
cascased_task_list =[
    'Caption + Grounding', 'Detailed Caption + Grounding', 'More Detailed Caption + Grounding', 'More Detailed Caption + Grounding + Dense Region Caption'
]
